...
def submit_file(sessionheaders, ifile, filename, profileID, url_base):
...
def submit_url(sessionheaders, suburl, profileID, url_base):
    payload = {'data': {'vmProfileList': profileID, 'submitType': 1, 'url': suburl}}
    data = json.dumps(payload)

    try:
       r = requests.post(url_base + "fileupload.php", headers=sessionheaders, data={'data': data}, verify=False)
       response = r.json()
       for line in response['results']:
          taskid = line['taskId']
       return taskid
...
class MfeAtdConnector(BaseConnector):
    ...
    def _handle_detonate_file(self, param):
        ...
        try:
            ...
            atdurl = "https://" + atd_ip + "/php/"
            headers = sessionsetup(creds, atdurl)
            taskid = submit_file(headers, filepath, filename, atd_profile, atdurl)
            while True:
               try:
    ...
    def _handle_detonate_url(self, param):
        ...
        try:
            ...
            atdurl = "https://" + atd_ip + "/php/"
            headers = sessionsetup(creds, atdurl)
            taskid = submit_url(headers, atd_suburl, atd_profile, atdurl)
            while True:
               try:
                  report = get_report(headers, taskid, atdurl, itype)
                  print report
